...
const logoutUser = asyncHandler( async (req, res) => {
    await user.findByIdAndUpdate(
        req.newUser._id,
        {
            $unset: {               //Earlier we used '$set' with value of 'refreshToken' as 'undefined' but that was not working properly. So, later we found about '$unset' and used it as follows(This removes the field from the document).
                refreshToken: 1
            }
        },
        {
...
const getCurrentUser = asyncHandler(async (req, res) => {
    ...
    .status(200)
    .json(
        new ApiResponse(200, req.user, "Current user fetched successfully")         // prone to error
    )
})
...
const updateUserAvatar = asyncHandler(async (req, res) => {
    ...
    // delete old avatar image -> assignment
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
    ...
    // delete old cover image -> assignemnt
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
    ...
    // We are starting aggregation pipelines from this point.
    const channel = await user.aggregate([
        ...
        },
        {
            $addFields: {
                ...
                isSubscribed: {
                    $cond: {
                        if: {$in: [req.user?._id, "$subscribers.subscriber"]},         // prone to error because of 'user'
                        then: true,
                        else: false
                    }
                }
    ...
    ])      // Try console logging this 'channel' and read or understand the output it give's.
...
const getWatchHistory = asyncHandler(async (req, res) => {
    const whUser = await user.aggregate([
        {
            $match: {
                _id: new mongoose.Types.ObjectId(req.user._id)          //prone to error because of 'user'
            }
        },
        {
            $lookup: {
                ...
                foreignField: "_id",
                as: "watchHistory",
                pipeline: [             //From here we are using sub-pipelines.
                    {
                        $lookup: {
                            from: "user",
                            localField: "owner",
                            foreignField: "_id",
                            as: "owner",
                            pipeline: [         //This is another sub-pipeline for the owner field.
                                {
                                    $project: {
                                        fullName: 1,
                                        username: 1,
